...
class DCGANUpdater(chainer.training.StandardUpdater):
    ...
    def update_core(self):
        ...
        x_real, labels = self.converter(
            batch, self.device)  # self.converter() is concat_example()
        x_real, labels = Variable(x_real), Variable(labels)
        ...
        y_real = dis(x_real, labels, 10)  # Genuine image estimation result

        z = Variable(xp.asarray(
            gen.make_hidden(batch_size)))  # genertate z random vector  xp.asarrayでcupy形式に変更する
        x_fake = gen(z, labels, 10)  # genertate fake data by generator
        y_fake = dis(x_fake, labels, 10)  # Estimation result of fake image
